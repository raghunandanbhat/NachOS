...
ExceptionHandler(ExceptionType which)
{
  ...
  int temp;
  int threadId = -1;
  static map<int, Thread*> threadTable;
  ...
  switch (which) {
          ...
          //printf("Exec() system call is called\n");
	{
		...
		int i = 0;
		int ch;
		if(kernel->machine->ReadMem(start, 1, &ch)){
			while((char)ch != '\0') 
			{
				progName[i] = (char)ch;
				i++;
				start++;
				kernel->machine->ReadMem(start, 1, &ch);
			}
			progName[i] = '\0';
			char *prog = strdup(progName); //create a duplicate pointer to the progName
			//Create a new thread and Allocate some address space to run user program
			Thread *thread = new Thread(prog);
			AddrSpace *space = new AddrSpace;
			ASSERT(space != (AddrSpace *)NULL);
			if(space->Load((char*)prog)){ //load program into the address space
				threadId = threadId + 2; //assign threadId
				threadTable.insert(pair<int, Thread*>(threadId, thread)); //put thread on thread table
				kernel->machine->WriteRegister(2, (int)threadId);//return threadId
				thread->Fork((VoidFunctionPtr) SysExec, (AddrSpace *)space);//Execute
			}else{
          ...
          //printf("Join() system call is called\n");
	{
		...
		int tid = (int)kernel->machine->ReadRegister(4);
		//find thread by thread id and attach the current thread to its waiting list
		auto itr = threadTable.find(tid);
		if(itr != threadTable.end()){
			Thread *execThread = itr->second;
			execThread->threadWaitList.Append(curThread);
			//if caller thread is failed to attach itslef to the waiting list, return -1
			if(execThread->threadWaitList.IsEmpty()){
		...
		curThread->Sleep(FALSE);
		//successful join, return 0
