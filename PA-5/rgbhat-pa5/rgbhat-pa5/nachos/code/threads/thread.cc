...
Thread::Finish ()
{
    ...
    //Thread is finishing, wake up the wait listed threads and put them on ready queue
    while(!kernel->currentThread->threadWaitList.IsEmpty()) {
	Thread *wakeupT = kernel->currentThread->threadWaitList.RemoveFront();
	kernel->scheduler->ReadyToRun(wakeupT);
    }
