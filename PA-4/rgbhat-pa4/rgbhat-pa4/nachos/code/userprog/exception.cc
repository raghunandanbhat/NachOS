...
ExceptionHandler(ExceptionType which)
{
  ...
  switch (which) {
        ...
        case SC_Exit:
	{
          ...
          //arg1 stores exit status value and arg1 is in register r4 
	  int exit_status = kernel->machine->ReadRegister(4);
	  if(exit_status == 0){
        ...
        case SC_Write:
	  {
		//printf("Write system call made by %s\n", kernel->currentThread->getName());
	  	//printf("The first parameter is %d, the second parameter is %d, the third parameter is %d\n", (int)kernel->machine->ReadRegister(4), (int)kernel->machine->ReadRegister(5), (int)kernel->machine->ReadRegister(6));
	  	//read parameters to the write system call
          	int source = (int)kernel->machine->ReadRegister(4);
	  	int size = (int)kernel->machine->ReadRegister(5);
	  	int dest = (int)kernel->machine->ReadRegister(6);
	  	int byte_count = 0;
   	  	//read each memeory location and write the contents of the memory location
	  	for(int i = 0; i < size; i++){
	  		if(kernel->machine->ReadMem(source, 1, &dest)){
				printf("%c", dest);
				source = source + 1;
				byte_count++;
			}
	  	}
	  	//write the number of bytes written to console output to register 2
	  	kernel->machine->WriteRegister(2, (int)byte_count); 
	  }
          break;
