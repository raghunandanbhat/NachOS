...
main(int argc, char **argv)
{
    ...
    List<char*> *userProgramList = new List<char*>(); //list to store multiple user programs
    ...
    for (i = 1; i < argc; i++) {
	    ...
	    ASSERT(i + 1 < argc);
	    //append userProgName at the end of list
	    userProgName = argv[i + 1];
	    userProgramList->Append(userProgName);
		//cout << userProgName << " append successful\n";
	    i++;
	}
    ...
    //iterate the list if not empty list
    if (!userProgramList->IsEmpty()){
	//remove from front one by one
	int userProgramCount = userProgramList->NumInList();
	for(int i=0; i<userProgramCount; i++){
		cout << "Program ["<< i <<"] = " << userProgramList->RemoveFront() <<"\n";
	}
    }
